...
# ╔═╡ 96015f97-3887-4b4f-9a1c-f80cd2a7acb9
begin
	using Plots, Colors,ColorSchemes, Images
	using PlutoUI
	# Take this out if you don't want to install and load CUDA:
	using CUDA
end

# ╔═╡ c778915a-8fa2-49e2-82ac-df4b42585aac
md"""
Enable checkbox to use run on GPU via CUDA:
"""

# ╔═╡ 2697e4c2-89ae-4a59-a6ec-b40c0f15601f
@bind use_cuda CheckBox(default = false)

# ╔═╡ e4e3350e-dbae-460c-9097-cc6108573f27
f(z, c) = @fastmath z^2 + c;

# ╔═╡ b5cee24c-7d4c-40c1-82fb-958da7ea4289
function n_f(z, c, nmax)
    n = 0
...
# ╔═╡ e439845a-be4c-467e-88d2-01b5ab80f6c0
html"""
<style> input[type*="range"] { width: 100%; } </style>
"""

# ╔═╡ f2b7defa-4cc8-4483-a142-9f4c8299b149
begin
	T = Float32;
	# Whether we compute on GPU or GPU just depends on the array type:
	N = if use_cuda
    	CuArray{Int}(undef, 500, 500)
	else
		Array{Int}(undef, 500, 500)
	end
	N_plotbuf = Array{Int}(undef, size(N))
end;

# ╔═╡ 43bd0a97-2efd-4112-a277-359b312c3c2f
begin
	r_re = range(T(-2), T(2), length = size(N, 2))
	r_im = range(T(-2), T(2), length = size(N, 1))
end;

# ╔═╡ 8b3c7e2b-62c0-4de6-a015-49c4e07a099e
md"""
...
# ╔═╡ 6ff8c33d-1408-4ca5-a419-5a53f1debebd
@bind n_iterations Select([10^3, 10^4, 10^5])

# ╔═╡ dc55c70e-bf2d-4b48-b5ed-a096c8e891ee
md"""
Set constant `c`:
"""

# ╔═╡ 82d5d31a-7334-48df-bf15-288a740b76d8
@bind c_re Slider(range(-2.5, 1, length = 1000); default = -0.786)

# ╔═╡ fae5d7bb-611a-4b82-8adc-29002668e055
@bind c_im Slider(range(-1.5, 1.5, length = 1000); default = 0.147)

# ╔═╡ 0560f1b0-5212-4827-ab57-55de84d41685
c = T(c_re) + T(c_im) * im

# ╔═╡ 7e4d0a18-f678-4e38-9a02-38935962431c
begin
	@time @. N = n_f(r_re' + r_im * im, c, n_iterations)

	N_plot = copyto!(N_plotbuf, N)
	#heatmap(N_plot, ratio = 1, format = :jpg)
	get.(Ref(ColorSchemes.magma), N_plot .* inv(maximum(N_plot)))
end

# ╔═╡ b11ec2b6-52ad-49b6-b85c-baae7522b2d5
maximum(N_plot)
...
manifest_format = "2.0"
project_hash = "13692a28212f3921b540cb1f557ae52bfe66a248"
